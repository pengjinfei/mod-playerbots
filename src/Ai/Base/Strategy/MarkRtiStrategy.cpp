...
void TrashCcPullStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    // 副本策略同时挂在 combat / non-combat 两个引擎上，所以这四条既管开怪前的指派与首次上控，
    // 也管战斗中的挪骷髅与重新上控。标记是瞬时的、放最高；上控放在移动之上、打断与紧急之下——
    // 开怪前它是这一刻唯一要做的事，战斗中则不能压过躲尖刺这类保命走位(ACTION_MOVE + 5)。
...
// 这就是「被控的最后杀」。
void TrashCcPullStrategy::AppendTargetExclusions(GuidSet& exclusions, TargetValueExclusionType type)
{
